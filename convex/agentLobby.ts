...
function normalizeLobbyMessageText(input: string) {
  return input.replace(/\s+/g, " ").trim();
}

async function buildLobbySnapshot(ctx: any, user: any, limit: number) {
...
export const getLobbySnapshot = query({
  ...
  handler: async (ctx, args) => {
    ...
    return buildLobbySnapshot(ctx, user, limit);
  },
});

export const getLobbySnapshotAsAgent = query({
  args: {
...
export const postLobbyMessage = mutation({
  ...
  handler: async (ctx, args) => {
    const user = await requireUser(ctx);
    const normalizedText = normalizeLobbyMessageText(args.text);
    if (!normalizedText) {
...
export const postLobbyMessageAsAgent = mutation({
  args: {
    agentUserId: v.id("users"),
    text: v.string(),
    source: v.optional(v.union(v.literal("agent"), v.literal("retake"), v.literal("system"))),
